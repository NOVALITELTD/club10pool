...
async function uploadImageToCloudinary(buffer: Buffer, fileName: string, investorId: string): Promise<string> {
...
async function uploadPdfToSupabase(buffer: Buffer, fileName: string, investorId: string): Promise<string> {
  const path = `kyc/${investorId}/${fileName}.pdf`
...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const field of allFields) {
      ...
      if (file.size > MAX_FILE_SIZE) return error(`${field} exceeds 5MB limit`)
      ...
      if (field === PROOF_FIELD) {
        ...
        if (!ALLOWED_IMAGE_TYPES.includes(mimeType)) {
          return error(`${field} must be an image (JPG, PNG, WEBP)`)
        }
      }
      ...
      if (isPdf) {
        const url = await uploadPdfToSupabase(buffer, field, investorId)
        results[field] = url
      } else {
        const url = await uploadImageToCloudinary(buffer, field, investorId)
        results[field] = url
      }
    }
