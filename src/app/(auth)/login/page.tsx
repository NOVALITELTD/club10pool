...
function LoginPageContent() {
  ...
  async function handleLogin(e: any) {
    ...
    try {
      ...
      // Auto-join referral pool if pending
      const pendingRef = localStorage.getItem('pendingReferral')
      if (pendingRef && member.kycStatus === 'APPROVED') {
        await fetch('/api/referrals/join', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${d.data.token}` },
  ...
  async function handleRegister(e: any) {
    ...
    try {
      const r = await fetch('/api/auth/register', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, referralCode: localStorage.getItem('pendingReferral') || undefined }),
      })
      const d = await r.json()
